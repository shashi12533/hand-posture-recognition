% Run 3 types of divergences on 2D data
...
qDist{1,1} = 0.3; qDist{1,2} = [0 0]'; qDist{1,3} = eye(2);
qDist{2,1} = 0.3; qDist{2,2} = [3 0]'; qDist{2,3} = eye(2);
qDist{3,1} = 0.4; qDist{3,2} = [8 0]'; qDist{3,3} = eye(2);
...
for shift = shift_list
    pDist{1,1} = 0.3; pDist{1,2} = [0 0+shift]'; pDist{1,3} = eye(2);
    pDist{2,1} = 0.3; pDist{2,2} = [3 0+shift]'; pDist{2,3} = eye(2);
    pDist{3,1} = 0.4; pDist{3,2} = [8 0+shift]'; pDist{3,3} = eye(2);
    ...
    x1 = [-15:dx1:23];
    x2 = [-15:dx2:15];
    [xI,yI] = meshgrid(x1,x2);
    qX = pdf(q_GMM_obj,[xI(:),yI(:)]);
    pX = pdf(p_GMM_obj,[xI(:),yI(:)]);
...
print('-depsc','-r200',['compareDiv_2D.eps']);
...
print('-depsc','-r200',['compareRuntime_2D.eps']);